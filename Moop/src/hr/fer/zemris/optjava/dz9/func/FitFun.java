...
public class FitFun implements IMoopOptFun{
	...
	public FitFun(MOOPProblem moopProblem, boolean objective) {
	...
	public LinkedList<LinkedList<NSGASolution>> getParetoFronts(
		...
		LinkedList<LinkedList<NSGASolution>> paretoFronts = new LinkedList<>();
		
		for(LinkedList<Integer> fronts : this.paretoFronts){
			LinkedList<NSGASolution> solutions = new LinkedList<>();
			for(Integer position : fronts){
				solutions.add(population.get(position));
			}
			paretoFronts.add(solutions);
		}
